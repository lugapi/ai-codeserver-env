...
> The base `docker-compose.yml` is optimized for Coolify (no host port, magic password variable).
...
Both work. **Docker Compose is recommended** for this repo because `docker-compose.yml` already declares everything Coolify would otherwise ask you to configure manually:

| | **Docker Compose** (recommended) | **Dockerfile** (alternative) |
|---|----------------------------------|------------------------------|
| Persistent volumes | Defined in `docker-compose.yml` — created automatically | Must add 3 mounts manually in Coolify UI |
| `PASSWORD` | Auto-generated via `SERVICE_PASSWORD_CODESERVER` magic var | Must set manually (`openssl rand -base64 32`) |
| Health check | Defined in `docker-compose.yml` | Configure in Coolify UI |
| Port | `expose: 8080` in compose — Traefik routes traffic | Set **Ports Exposes** = `8080` in General tab |
| Local testing | Same `docker-compose.yml` + `docker-compose.local.yml` | `docker compose` with local override |
...
|---------------|-------|-------|
| **Build Pack** | `Docker Compose` | Coolify reads `docker-compose.yml` at repo root |
| **Base Directory** | `/` | Repository root |
| **Docker Compose Location** | `/docker-compose.yml` | Default if file is at root |
| **Domains** | `https://code.yourdomain.com` | Your FQDN with `https://` prefix |

> Coolify may auto-detect Docker Compose because `docker-compose.yml` exists at the root. If it picked Nixpacks or Dockerfile, **manually switch** to Docker Compose.

> You do **not** need to set Ports Exposes, Persistent Storage, or PASSWORD manually — they come from `docker-compose.yml`.
...
Persistent Storage:      (defined in docker-compose.yml — no manual setup)
